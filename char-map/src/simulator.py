...
class Simulator:
    def __init__(self, cfg, policy):
        ...
        elif cfg.physics_model == "regime":
            self.physics = RegimeSwitchingAdvection(
                c0=cfg.velocity,
                amplitude=cfg.variable_amplitude,
                omega=cfg.variable_omega,
                L=cfg.domain_length,
    ...
    def run(self):
                    ...
                    self.rows.append(
                        {
                            ...
                            "timestep": t,
                            "cycle": self.cycle,
                            "consumer_pe": r.consumer_pe,
                            "block_id": r.block_id,
                            "producer_pe": r.producer_pe,
                            "hit": res.hit,
                            "stall_cycles": res.stall_cycles,
                            "step_stall_cycles": step_stall,
                            "transfer_hops": res.transfer_hops,
                            "prefetched_hit": res.prefetched_hit,
                        }
                    )
        ...
        s = summarize_accesses(
            ...
            total_bytes=self.hardware.total_bytes,
            total_hops=self.hardware.total_hops,
            prefetch_bytes=self.hardware.total_prefetch_bytes,
            evictions=self.hardware.evictions,
        )
